...
export default function SocketFormWrapper({ filter }: Props) {
  const [initialValues, setInitalValues] = useState<SocketInitialProps>({
    obyektId: 0,
    modelId: 0,
    count: "",
    info: "",
  ...
  useEffect(() => {
    if (productId) {
      SocketApi.getOneSocket({ id: productId }).then((r) => {
        const json = {
          ...r?.data,
          modelId: {
            label: r?.data?.model,
